...
def fork_rng(devices=None, enabled=True, _caller="fork_rng", _devices_kw="devices"):
    ...
    Arguments:
        devices (iterable of CUDA IDs): CUDA devices for which to fork
            ...
            of machines, since this function will run very slowly in that case.
    ...
    if devices is None or devices is True:
        suppress_warning = devices is True
        num_devices = torch.cuda.device_count()
        if num_devices > 1 and not suppress_warning and not _fork_rng_warned_already:
            warnings.warn(
                ("CUDA reports that you have {num_devices} available devices, and you "
                 ...
                 "to 'True'.").format(num_devices=num_devices, caller=_caller, devices_kw=_devices_kw))
