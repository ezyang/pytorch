...
def forward_improved(fn, input, hx, weight):
    with torch.cuda.device_of(input):
        if fn.mode == cudnn.CUDNN_LSTM:
            hx, cx = hx
        else:
...
def backward_grad_improved(fn, input, hx, weight, output, grad_output, grad_hy):
    with torch.cuda.device_of(input):
        if fn.mode == cudnn.CUDNN_LSTM:
            hx, cx = hx
            grad_hy, grad_cy = grad_hy
...
def backward_weight_improved(fn, input, hx, output, weight):
    with torch.cuda.device_of(input):
        if fn.mode == cudnn.CUDNN_LSTM:
            hx, cx = hx
        else:
