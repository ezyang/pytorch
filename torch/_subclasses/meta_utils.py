...
class MetaConverter:
    ...
    def meta_tensor(
        self, t, shape_env=None, callback=lambda t: t(),
        ...
        def sym_sizes_strides_storage_offset(t):
            if make_symbolic:
                return shape_env.create_symbolic_sizes_strides_storage_offset(t, sname=sname)
        ...
        if self.get_tensor_memo(t) is None:
            with torch.inference_mode(t.is_inference()):
                if t.is_sparse:
                    ...
                    base = self.meta_tensor(t._base, shape_env, callback, sname=f"{sname}._base")
                ...
                if safe_grad(t) is not None:
                    r.grad = self.meta_tensor(safe_grad(t), shape_env, callback, sname=f"{sname}.grad")
    ...
    def __call__(
        self, t, shape_env=None, *, callback=lambda t: t(), ignore_subclass=False, sname=None
    ):
        ...
        if (
            ...
            if any(
                ...
                with ctx:
                    r = self.meta_tensor(t, shape_env=shape_env, callback=callback, sname=sname)
