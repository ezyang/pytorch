...
struct TensorDescriptor
{
  ...
  TensorDescriptor() : desc(nullptr), ptr(nullptr) {
    CUDNN_CHECK(cudnnCreateTensorDescriptor(&desc));
  }
  /* implicit */ TensorDescriptor(const at::Tensor& t, int64_t pad = 0) : desc(nullptr), ptr(t.data_ptr()) {
    CUDNN_CHECK(cudnnCreateTensorDescriptor(&desc));
    set(t, pad);
  }
  TensorDescriptor(const TensorDescriptor&) = delete;
  TensorDescriptor(TensorDescriptor&& ref) { desc = ref.desc; ref.desc = nullptr; }
  ~TensorDescriptor() {
...
struct FilterDescriptor
{
  ...
  FilterDescriptor() : desc(nullptr), ptr(nullptr) {
    CUDNN_CHECK(cudnnCreateFilterDescriptor(&desc));
  }
  /* implicit */ FilterDescriptor(const at::Tensor& t, int64_t pad = 0) : desc(nullptr), ptr(t.data_ptr()) {
    CUDNN_CHECK(cudnnCreateFilterDescriptor(&desc));
    set(t, pad);
  }
