...
def gen_create_out_helper(backend_index: BackendIndex) -> List[str]:
    ...
    elif backend_index.dispatch_key is DispatchKey.QuantizedCPU:
        # NB: can't conveniently use native:: as this accepts the arguments literally
        empty_impl = "at::_empty_unknown_quantized"
        empty_strided_impl = None
    elif backend_index.dispatch_key == DispatchKey.CompositeExplicitAutograd:
        empty_impl = "at::_empty_maybe_quantized"
        empty_strided_impl = "at::_empty_strided_maybe_quantized"
    else:
        return []

    if empty_strided_impl is not None:
        empty_strided_call = f"return {empty_strided_impl}(sizes, strides, {empty_options});"
    else:
        empty_strided_call = (
            'TORCH_CHECK(0, "quantized tensors do not support striding, '
