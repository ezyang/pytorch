...
std::tuple<Tensor, Tensor, Tensor> batchnorm_double_backward(
  ...
  Tensor gamma_expanded;  // NB: when undefined, interpret as 1
  ...
  auto sigma2_eps_neg_1_2 = unsqueeze_dim1(make_variable(training ? save_std : (running_var + eps).pow(-0.5)), input);
  ...
  if (ggI.defined() && training) {
    auto ggI_sum = sum_exclude_dim1(ggI * input_sub_mu);
  ...
  if (affine && ggI.defined()) {
    if (training) {
      ...
      gG = ggI * first_back_grad_input(gO, Scalar(1).toTensor());
